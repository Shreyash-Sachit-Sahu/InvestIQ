...
class PortfolioOptimizer:
    ...
    def optimize_all_strategies(self):
        ...
        def neg_sharpe(w):
            return -self.calculate_portfolio_metrics(w)[2]

        def port_variance(w):
            return np.dot(w.T, np.dot(self.returns_data.cov() * 252, w))

        res = minimize(neg_sharpe, initial, bounds=bounds, constraints=constraints)
        ms = res.x
        mr, mv, msharpe = self.calculate_portfolio_metrics(ms)
        portfolios['Max Sharpe'] = {'weights': ms, 'return': mr, 'volatility': mv, 'sharpe_ratio': msharpe}
        ...
        portfolios['Min Variance'] = {'weights': mvw, 'return': pvr, 'volatility': pvv, 'sharpe_ratio': pvsharpe}

        vol = self.returns_data.std() * np.sqrt(252)
        invvol = 1 / vol
        rp_w = invvol / np.sum(invvol)
        rpr, rpv, rps = self.calculate_portfolio_metrics(rp_w)
        portfolios['Risk Parity'] = {'weights': rp_w, 'return': rpr, 'volatility': rpv, 'sharpe_ratio': rps}

        ew = np.array([1/n_assets] * n_assets)
        ewr, ewv, ews = self.calculate_portfolio_metrics(ew)
        portfolios['Equal Weight'] = {'weights': ew, 'return': ewr, 'volatility': ewv, 'sharpe_ratio': ews}
